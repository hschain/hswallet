		...
		<view class="textWrapper">
			...
			<u-form :model="form" ref="uForm" :error-type="errorType">
				<u-form-item class="inputText" prop="value" :border-bottom="false">
					<u-input class="inputValue" v-model="form.value" type="textarea" :border="true" :height="180"
						:auto-height="true" maxlength="300" :clearable="false" :placeholder="holder"
					...
					if (valid) {
						...
						uni.showToast({
							...
							success() {
								uni.hideLoading();
								setTimeout(() => {
									_this.$store.commit('SET_WALLETNAME', _this.$store.state
										.walletType + `-${_this.nameIndex}`)
									uni.setStorageSync('userAddress', addr)
									let accounts = _this.accountList ? _this.secret.decrypt(_this
										.accountList) : {}
									let userWallet = _this.accountList ? _this.$store.state
										.userWallet : [{
											addr,
					...
					uni.showToast({
						...
						success() {
							...
							setTimeout(() => {
								...
								let accounts = _this.accountList ? _this.secret.decrypt(_this
									.accountList) : {}
								let userWallet = _this.accountList ? _this.$store.state.userWallet : [{
									addr,
