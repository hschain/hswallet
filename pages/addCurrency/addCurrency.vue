<template>
	<view class="addCurrency">
		...
		<view class="inputs">
			...
			<u-input v-model="keyword" type="text" placeholder="请输入Token名称或合约地址" :height="96"
				:custom-style="{borderRadius:'24px',paddingLeft:'36px'}" @input="onSearch" :focus="true" />
		</view>
		...
		<view v-if="!keyword">
			...
			<view class="assetsList" v-if="Type.type == 'HST'">
				<view v-for="(item,index) in assetsList "
			...
			<view class="assetsList" v-if="Type.type == 'ETH'">
				<view v-for="(item,index) in ETHassetsList"
					:key="Type.type=='HST' ? item.denom : item.value" class="table ">
					<view class="tableWrapper">
						<view class="tableLeft">
							<image class="icon" v-if="Type.type=='HST'" src="../../static/common/logo.png" mode="">
			...
			<view class="assetsList" v-if="Type.type == 'HECO'">
				<view v-for="(item,index) in HECOassetsList"
					:key="Type.type=='HST' ? item.denom : item.value" class="table ">
					<view class="tableWrapper">
						<view class="tableLeft">
							<image class="icon" v-if="Type.type=='HST'" src="../../static/common/logo.png" mode="">
		...
		<view v-if="keyword">
			<view class="assetsList tokenList">
				...
				<view v-for="(item,index) in tokenList" :key="index" class="table">
					...
					<image class="assetsEdit" src="../../static/svg/ic_add.svg" mode="" @click="addAssets(index)">
					</image>
					<view class="border"></view>
				</view>
			</view>
						...
						uni.request({
							...
							success: (res) => {
								...
								tokens.map(async (item) => {
									if (item.split('\t')[1]) {
										uni.showLoading()
										let decimal = await _that.$wallet("ETH").getDecimal(
											item.split('\t')[1])
										_that.tokenList.push({
											...
											logo: "https://cn.etherscan.com/token/images/" +
												item.split('\t')[5],
											decimal: decimal
										})
										uni.hideLoading()
